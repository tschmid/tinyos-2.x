...
implementation
{
  ...
  async event void ReadNow.readDone(error_t result, uint16_t usActualPeriod)
  {
    if (result != SUCCESS) { /*call Leds.led2On();*/ }
    //avg = (call Counter.get() - startCounter);
  ...
  */
  ...
  {
    if (error != SUCCESS) {
      while (call AMSend.send(0xFF, &sendPkt, call Packet.payloadLength(msg)) != SUCCESS);
    } else {
      post send();
    }
  }
