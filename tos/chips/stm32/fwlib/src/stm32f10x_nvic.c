...
{
  return ((uint16_t)((SCB->ICSR & (uint32_t)0x003FF000) >> 0x0C));
}
...
ITStatus NVIC_GetIRQChannelPendingBitStatus(uint8_t NVIC_IRQChannel)
{
  ...
  tmp = ((uint32_t)0x01 << (NVIC_IRQChannel & (uint32_t)0x1F));
...
void NVIC_SetIRQChannelPendingBit(uint8_t NVIC_IRQChannel)
{
  ...
  *(uint32_t*)0xE000EF00 = (uint32_t)NVIC_IRQChannel;
}
...
void NVIC_ClearIRQChannelPendingBit(uint8_t NVIC_IRQChannel)
{
  ...
  NVIC->ICPR[(NVIC_IRQChannel >> 0x05)] = (uint32_t)0x01 << (NVIC_IRQChannel & (uint32_t)0x1F);
}
...
{
  return ((uint16_t)(SCB->ICSR & (uint32_t)0x3FF));
}
...
void NVIC_SystemHandlerConfig(uint32_t SystemHandler, FunctionalState NewState)
{
  ...
  tmpreg =  (uint32_t)0x01 << (SystemHandler & (uint32_t)0x1F);
...
void NVIC_ClearSystemHandlerPendingBit(uint32_t SystemHandler)
{
  ...
  SCB->ICSR |= ((uint32_t)0x01 << (tmp - 0x01));
}
