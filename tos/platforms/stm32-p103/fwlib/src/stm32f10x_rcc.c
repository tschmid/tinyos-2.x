...
#define HSEStartUp_TimeOut        ((uint8_t)0xFF)
...
static volatile uint32_t StartUpCounter = 0;
...
void RCC_ITConfig(uint8_t RCC_IT, FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    ...
    *(volatile uint8_t *) 0x40021009 |= RCC_IT;
  }
  else
  {
    /* Perform Byte access to RCC_CIR[12:8] bits to disable the selected interrupts */
    *(volatile uint8_t *) 0x40021009 &= ~(uint32_t)RCC_IT;
  }
}
...
void RCC_LSEConfig(uint32_t RCC_LSE)
{
  ...
  /* Reset LSEON bit */
  *(volatile uint8_t *) BDCR_BASE = RCC_LSE_OFF;
  ...
  *(volatile uint8_t *) BDCR_BASE = RCC_LSE_OFF;
  ...
  switch(RCC_LSE)
  {
    case RCC_LSE_ON:
      /* Set LSEON bit */
      *(volatile uint8_t *) BDCR_BASE = RCC_LSE_ON;
      break;
      
    case RCC_LSE_Bypass:
      /* Set LSEBYP and LSEON bits */
      *(volatile uint8_t *) BDCR_BASE = RCC_LSE_Bypass | RCC_LSE_ON;
      break;            
...
void RCC_MCOConfig(uint8_t RCC_MCO)
{
  ...
  *(volatile uint8_t *) 0x40021007 = RCC_MCO;
}
...
void RCC_ClearITPendingBit(uint8_t RCC_IT)
{
  ...
  /* Perform Byte access to RCC_CIR[23:16] bits to clear the selected interrupt
     pending bits */
  *(volatile uint8_t *) 0x4002100A = RCC_IT;
}
